...
def get_s_next(s, a, Q, epsilon, pi_0, ver):
    direction = ["up","right","down","left"]
    ...
    if next_direction == "up":
        s_next = s-number # When the agent moves up, its state value is reduced by 3
    elif next_direction == "right":
        s_next = s+1 # When the agent moves right, its state value is increased by 1
    elif next_direction == "down":
        s_next = s+number # When the agent moves down, its state value is increased by 3
    elif next_direction == "left":                           
...
def Sarsa(s, a, r, s_next, a_next, Q, eta, gamma,ver):
    if ver == "small":
        number = 8
    elif ver == "large":
        number = 49
...
def goal_maze_ret_s_a_Q(Q, epsilon, eta, gamma, pi, ver):
    ...
    while (1): # Repeat until the agent reaches(도달하다) the target point(goal point)
        ...
        s_next = get_s_next(s, a, Q, epsilon, pi, ver) # get the state of the next step
