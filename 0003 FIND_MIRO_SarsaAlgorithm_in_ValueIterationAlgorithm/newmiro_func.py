...
def get_s_next(s, a, Q, epsilon, pi_0):
    ...
    if next_direction == "up":
        s_next = s-10 # When the agent moves up, its state value is reduced by 3
    elif next_direction == "right":
        s_next = s+1 # When the agent moves right, its state value is increased by 1
    elif next_direction == "down":
        s_next = s+10 # When the agent moves down, its state value is increased by 3
    elif next_direction == "left":                           
...
def goal_maze_ret_s_a_Q(Q, epsilon, eta, gamma, pi):
    ...
    while (1): # Repeat until the agent reaches(도달하다) the target point(goal point)
        ...
        # End decision
        if s_next == 49: # End if agent reches the target point
            break
        else:
            s = s_next
